<#list page.content() as group>
    <details class="measurement-group bg-white rounded-xl border border-slate-200 overflow-hidden"
             <#if (isFirstPage && group?is_first)>open</#if>>
        <summary class="bg-slate-50/50 px-4 py-3 border-b border-slate-100 flex justify-between items-center cursor-pointer hover:bg-slate-100 transition-colors list-none">
            ...
            <svg class="w-4 h-4 text-slate-400 transform transition-transform duration-200 chevron-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="divide-y divide-slate-100">
            <#list group.items() as item>
                <div class="p-4 hover:bg-slate-50/30 transition-colors">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        <div class="flex items-center gap-2 sm:gap-1.5 mt-3 sm:mt-0 pt-3 sm:pt-0 border-t sm:border-t-0 sm:border-l border-slate-100 sm:pl-4">
                            ...
                            <form action="/measurements/${item.id()}" method="post" onsubmit="return confirm('Удалить запись?');" class="flex-1 sm:flex-none">
                                <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                                <input type="hidden" name="_method" value="delete">
                                <button type="submit" class="w-full flex items-center justify-center gap-2 sm:gap-0 px-3 py-2.5 sm:p-2 bg-red-50 sm:bg-transparent text-red-600 sm:text-slate-400 hover:text-red-700 sm:hover:text-red-500 hover:bg-red-50 rounded-lg transition-all text-xs font-bold sm:font-normal">

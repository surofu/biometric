...
<@layoutMacros.layout title="Врачи и специализации — Биометрик" selectedPage="5">
        ...
        <div class="container max-w-2xl mx-auto px-4 pt-8 pb-20">
            ...
            <div class="flex items-center gap-3 mb-6">
                ...
                <div>
                    ...
                    <h1 class="text-xl font-bold text-slate-900 leading-none">Врачи и специализации</h1>
                </div>
            </div>
            ...
            <#if doctors?? && doctors?has_content>
                ...
                <div class="mb-4">
                    <span class="px-2.5 py-1 bg-slate-100 rounded-lg text-xs font-semibold text-slate-500"
                          id="doctors-count">${doctors?size} специализаций</span>
                </div>

                <!-- List -->
                <div class="space-y-2" id="doctors-list">
                    <#list doctors as doc>
                        <a href="/reference/doctors/${doc.id}"
                           class="doctor-item group flex items-center gap-4 p-4 border border-slate-100 rounded-xl bg-white hover:border-emerald-200 hover:bg-slate-50/30 transition-all duration-150"
                           data-name="${doc.name?lower_case}">
                            ...
                            <h3 class="flex-1 font-semibold text-slate-800 text-sm group-hover:text-emerald-700 transition-colors">${doc.name}</h3>
            ...
            <#else>
                <div class="text-center py-16">
                    ...
                    <p class="text-slate-400 text-sm">Специализации не найдены</p>
                </div>
            </#if>
        </div>
    </div>
    ...
    <script>
        function searchDoctors(query) {
            ...
            if (countEl) countEl.textContent = visible + ' специализаций';

<#macro navbar selectedPage="-1">
    <nav class="fixed flex justify-center bottom-0 left-0 w-full h-fit z-10 md:hidden px-4 py-2">
    ...
    <nav class="hidden md:block fixed top-0 left-0 w-full bg-white/20 backdrop-blur-sm border-b border-slate-200 z-10">
        <div class="container max-w-6xl mx-auto px-4">
            <div class="flex items-center justify-between h-16">
                <a href="/" class="flex items-center gap-2 text-emerald-600">
                    <svg width="32" height="32" class="text-emerald-500">
                        <use href="/favicon.svg#content" width="32" height="32"></use>
                    </svg>
                ...
                <div class="flex items-center gap-1">
                    <a href="/"
                       class="px-4 py-2 rounded-lg hover:bg-emerald-50 transition-colors <#if selectedPage="0">text-emerald-600 font-medium bg-emerald-50<#else>text-gray-700</#if>">
                        Главная
                    </a>
                    <a href="/measurements"
                       class="px-4 py-2 rounded-lg hover:bg-emerald-50 transition-colors <#if selectedPage="1">text-emerald-600 font-medium bg-emerald-50<#else>text-gray-700</#if>">
                        Показатели
                    </a>
                    <a href="/measurements/new"
                       class="px-4 py-2 rounded-lg hover:bg-emerald-50 transition-colors <#if selectedPage="2">text-emerald-600 font-medium bg-emerald-50<#else>text-gray-700</#if>">
                        Добавить
                    </a>
                    <a href="/analytics"
                       class="px-4 py-2 rounded-lg hover:bg-emerald-50 transition-colors <#if selectedPage="3">text-emerald-600 font-medium bg-emerald-50<#else>text-gray-700</#if>">
                        Аналитика
                    </a>
                    <a href="/profile"
                       class="px-4 py-2 rounded-lg hover:bg-emerald-50 transition-colors <#if selectedPage="4">text-emerald-600 font-medium bg-emerald-50<#else>text-gray-700</#if>">
                        Профиль
                    </a>
                </div>
                ...
                <form action="/logout" method="post" class="ml-4" onsubmit="return confirm('Вы уверены, что хотите выйти из аккаунта?');">
                    ...
                    <button type="submit"
                            class="flex items-center gap-2 px-4 py-2 text-gray-700 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors cursor-pointer keysetCursor">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"

...
<@adminLayoutMacros.adminLayout title="Доктора" selectedPage="doctors">
    <div class="py-6 sm:py-8 pb-16">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div class="min-w-0">
                <h1 class="text-xl sm:text-2xl font-semibold text-slate-800 tracking-tight break-words">Доктора</h1>
            ...
            <div class="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
                ...
                <a href="/admin/doctors/add" class="w-full sm:w-auto bg-emerald-600 text-white px-6 py-2 rounded-md text-sm font-semibold hover:bg-emerald-700 transition-colors flex items-center justify-center gap-2 whitespace-nowrap">
                    ...
                    <div class="p-4 search-item" data-search="${item.name()?lower_case}">
                        <div class="flex justify-between items-center gap-4">
                            <div class="min-w-0 flex-1">
                                <h3 class="text-sm font-bold text-slate-800 break-words leading-snug tracking-tight">${item.name()}</h3>
                                <p class="text-[10px] text-slate-400 font-mono mt-1 uppercase tracking-wider">ID: ${item.id()}</p>
                            </div>
                            <div class="flex gap-2 shrink-0">
                                <a href="/admin/doctors/${item.id()}/edit" class="p-2.5 bg-slate-100 text-slate-500 rounded-md hover:bg-slate-200 transition-colors"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" stroke-width="2"/></svg></a>
                                <form method="post" action="/admin/doctors/${item.id()}" onsubmit="return confirm('Удалить?')">
            ...
            <table class="hidden sm:table min-w-full">
                ...
                <#list doctors as item>
                    <tr class="hover:bg-slate-50/50 transition-colors search-item" data-search="${item.name()?lower_case}">
                        ...
                        <td class="px-6 py-4 text-sm font-semibold text-slate-800 break-words tracking-tight">${item.name()}</td>
                        <td class="px-6 py-4 text-center">
                            <div class="flex justify-center gap-2">
                                <a href="/admin/doctors/${item.id()}/edit" class="p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-md transition-all"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" stroke-width="2"/></svg></a>
                                <form method="post" action="/admin/doctors/${item.id()}" onsubmit="return confirm('Удалить?')">
